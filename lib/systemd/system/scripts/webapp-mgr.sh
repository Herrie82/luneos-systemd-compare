...
export NETWORK_QUIET_TIMEOUT=3
...
export ENABLE_BLINK_FEATURES=AudioFocusExtension,MediaSourceIsSupportedExtension,MediaTimelineOffset,UMSExtension,LocalResourceCodeCache,CustomEventExtension,WebCodecs,MediaStreamInsertableStreams
...
export WAM_COMMON_SWITCHES=" \
   ...
   --force-max-touch-points=10 \
    ...
    --network-quiet-timeout=$NETWORK_QUIET_TIMEOUT \
    --noerrdialogs \
    --no-sandbox \
...
export WAM_MEM_FLAGS=" \
    --local-storage-limit-per-second-level-domain=10 \
...
exec $WAM_EXE_PATH $WAM_SWITCHES $WAM_EXTRA_SKIA_CACHE_SWITCHES $WAM_EXTRA_GPU_TUNING_SWITCHES $WAM_WATCHDOG_RENDERER_TIMEOUT_SWITCHES $WAM_MEM_FLAGS $WAM_EXTRA_FLAGS  --webos-js-flags="$WAM_JS_FLAGS"
